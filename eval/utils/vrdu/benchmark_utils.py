...
# from match_utils import AddressMatch  # pylint: disable=unused-import
# from match_utils import DateMatch  # pylint: disable=unused-import
# from match_utils import DefaultMatch  # pylint: disable=unused-import
# from match_utils import Entity
# from match_utils import GeneralStringMatch  # pylint: disable=unused-import
# from match_utils import Match
# from match_utils import NameMatch  # pylint: disable=unused-import
# from match_utils import NumericalStringMatch  # pylint: disable=unused-import
# from match_utils import PriceMatch  # pylint: disable=unused-import
...
class DataUtils:
  ...
  def load_entity_name_info(
    ...
    # Group the entity names with the same appearance patterns 如果访问一个不存在的值，返回默认list 这里就是做一个聚合，将unrepeated的值放进去
  ...
  def load_dataset(
    ...
    with open(jsonl_filepath, 'r') as f:
      for line in f:
        ...
        filtered_data = [item for item in annotations[filename] if isinstance(item[0], str)]
        annotations[filename] = filtered_data
...
def evaluate(model_extractions,
  ...
  for entity_name in all_entity_names:
    if (entity_name == 'line_item'): continue
