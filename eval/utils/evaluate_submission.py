...
# 这里选择将所有waring忽略，不在控制台输出
warnings.filterwarnings("ignore")
# python3 evaluate_submission.py -g=gt/DUDE_val.json -s=submissions/DUDE_val.json
...
def evaluate_method_v1(gtJson, submJson, answer_types, anls_threshold, score_calibration):
    """
    ...
    """
    # 这里主要是看需不需要返回每一项的ANLS值，answer_types值默认是False
    show_scores_per_question_type = answer_types
    ...
    for gtObject in gtJson["data"]:
        ...
        if question_result < 0.5: false_list.append(perSampleMetrics[str(gtObject["questionId"])])
        else: true_list.append(perSampleMetrics[str(gtObject["questionId"])])
        row = row + 1
    true_list_save_path, false_list_save_path = 'true_list.json', 'false_list.json'
    ...
    if score_calibration:
        ...
        for q in perSampleMetrics:
            ...
            y_correct.append(int(m["anls"] >= anls_threshold))
...
def solve(ground_truth, submission_file, anls_threshold):
    gtJson, submJson = validate_data(ground_truth, submission_file)
    # Evaluate method
    results = evaluate_method_v1(gtJson, submJson, False, anls_threshold, False)
    # display_results(results, False)
    return results["result"]["anls"]
